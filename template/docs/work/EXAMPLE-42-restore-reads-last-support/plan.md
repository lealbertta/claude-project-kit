...
|----|--------|---------|--------|-----------|------------------------|
| 42.1 | Serialize the support id alongside the position. It is already stable and already unique; nothing new has to be minted. | `src/core/scene/serialize` | AC-1 | `SerializedStateCarriesSupport` | Drop the field on write — the round trip must fail, not fall back to a default |
| 42.2 | Restore seats against the recorded support instead of querying what is beneath. | `src/core/placement/restore` | AC-1, AC-2 | `RestoreReadsLastSupport` | Reinstate the beneath-query as the primary path — must fail even though every coordinate is still correct |
| 42.3 | Missing support: seat on the ground plane, emit `support-lost` once. | `src/core/placement/restore` | AC-3 | `RestoreEmitsOnceOnMissingSupport` | Emit twice; and separately, emit zero times — a test that only catches one of those is counting nothing |
| 42.4 | Migration: files written before this read as support-less and take the ground plane, which is what they already did. | `src/core/scene/migrations` | AC-1 | `LegacyFileRestoresWithoutSupport` | Bump the version without writing the migration — must fail loudly rather than reading a missing field as id `0` |

If you cannot name the mutation, the test is decoration and the plan is not
finished. 42.3 is the one that needed two: an emit-once criterion is two
independent failures and a single mutation only ever pins one of them.

42.2 is first in risk order rather than first in dependency order — 42.1 has to
land before it compiles, but 42.2 is the item that proves the whole idea works,
...
pasted objects through the same path. Paste gets 42.2 for free and AC-1's test
...
- **Schema / stored data:** yes — the serialized object gains `support`. Forward
  migration in 42.4; old files stay readable and lose nothing they had.
...
- **Compatibility:** 42.4 bumps the schema version, so files written after this
...
- **Lifecycle:** an object deleted while something rests on it already leaves a
  dangling reference. That is not new, but 42.3 is the first code that has to have
  an answer for it.

**One-way doors**

- **42.1 with 42.4.** Reverting the code does not unwrite files already on disk.
  A user who saves once on this build has a file carrying the bumped version, and
  the reverted build refuses it. Made as reversible as it can be — 42.4's reader
...
Nothing else here survives a revert. 42.2 and 42.3 are code paths only.
...
1. **Old files stopped opening.** The version bump landed without the migration and
   ...
   on whichever object happened to be first. → 42.4, and its mutation is exactly
   this failure.
...
and the `support-lost` event from 42.3 fires for the *missing*-support case, not
...
- **2025-03-07** — 42.5 added after the review pass: unpick the fixture helper in
  ...
  42.3 — the numbers are ids, the next free one is the one you get, and a commit
  already cites 42.3.
- **2025-03-06** — 42.3 assumed the restore path could emit directly. It cannot:
